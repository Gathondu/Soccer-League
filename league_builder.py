...
if __name__ == "__main__":
    ...
    #function that takes in a list of players and divides them into equal teams
    def get_teams(players):
        #check if number of experienced players is even
        if len(players) % 2 > 0:
        #check if number of experienced players is divisible by number of teams
            if len(players) % 3 == 0:
                while len(players) > 0:
    ...
    def write_letter(file,team_name,players):
        for player in players:
